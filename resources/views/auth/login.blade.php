<x-guest-layout>
    ...
    <div style="text-align: center; margin-bottom: 24px;">
        <h2 style="font-size: 24px; font-weight: 600; color: #202124; margin-bottom: 8px;">Selamat Datang Kembali</h2>
        <p style="color: #5f6368; font-size: 14px;">Silakan login untuk masuk ke kelas Anda</p>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="auth-input-group">
            ...
            <input id="email" class="auth-input" type="email" name="email" value="{{ old('email') }}" required autofocus autocomplete="username" placeholder="Masukkan email Anda">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
            <label for="remember_me" style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                <input id="remember_me" type="checkbox" name="remember" style="width: 16px; height: 16px; border-radius: 4px; border: 1px solid #dadce0;">
                <span style="font-size: 14px; color: #3c4043;">Ingat Saya</span>
            </label>

            @if (Route::has('password.request'))
                <a class="auth-link" style="font-size: 14px;" href="{{ route('password.request') }}">
                    Lupa password?
                </a>
            @endif
        </div>
